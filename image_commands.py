...
class image_commands():
    ...
    async def nickelback(self):
        ...
        await self.bot.say(message.channel, rand_song)
    ...
    async def harambe(self):
        ...
        await self.bot.say(message.channel, '/home/pi/thombot/pictures/harambe/' + random.choice(harambe_pics))

    @commands.command
    async def aliens(self):
        await self.bot.say(message.channel, '/home/pi/thombot/pictures/aliens.png')

    @commands.command
    async def slab(self):
        await self.bot.say(message.channel, 'RETURN THE SLAAAAB')

    @commands.command
    async def dva(self:
        await self.bot.say(message.channel, '/home/pi/thombot/pictures/dva.png')

    @commands.command
    async def mission_statement(self):
        await self.bot.say(message.channel, '/home/pi/thombot/pictures/thom_stargazer.jpg')

    @commands.command
    async def goo(self):
        await self.bot.say(message.channel, '/home/pi/thombot/pictures/goo.jpg')
